...
class PruebaVarianzaNormal(PruebaInterfaz):

    def __init__(self, desv_teorica: float, nivel_confianza: float = 0.95):
        self._nombre = "Prueba de la Varianza (Normal)"
        self._nivel_confianza = nivel_confianza
        self._varianza_teorica = desv_teorica ** 2  # sigma al cuadrado
    ...
    def ejecutar(self, numeros: list[float]) -> ResultadoPruebaDTO:
        ...
        # 1. Calcular varianza real de los datos muestrales
        media = sum(numeros) / n
        suma_cuadrados = sum((x - media) ** 2 for x in numeros)
        varianza_muestra = suma_cuadrados / (n - 1)

        # 2. Estadístico Chi-cuadrado para la normal: ((n - 1) * S^2) / (sigma^2)
        estadistico_chi = ((n - 1) * varianza_muestra) / self._varianza_teorica

        # 3. Límites críticos de Chi-cuadrado con n-1 grados de libertad
        gl = n - 1
